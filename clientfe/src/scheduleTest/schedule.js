...
const ScheduleComponent = () => {
    ...
    const handleOkCreate = () => {
        ...
        if (Math.ceil((onFormCreate.countStudnetExam) / (onFormCreate.roomPeopleMax)) !== onFormCreate.roomExam.length) {
    ...
    const changeMaxPeople = (e) => {
        ...
        const dataArray = []
        const numberCell = Math.ceil((onFormCreate.countStudnetExam) / (valueGet));
    ...
    const fun_disableData = (e) => {
        let check = false;
        if (onFormCreate.roomExam.length > Math.ceil((onFormCreate.countStudnetExam) / (onFormCreate.roomPeopleMax)) - 1) {
            check = true;
        }
            ...
            <Modal title="Tạo Lịch Thi" open={isModalOpenCreate} footer={null}>
                ...
                >
...

                    {
                        ...
                        <div>
                            <div className='count_exam'> Có tổng cộng {countStudnetExam} sinh viên đăng kí học môn</div>
                            <Form.Item
                                label="Hình thức thi"
                                name="form_exam"
                            >
                    ...
                    {onFormCreate.roomPeopleMax > 0 &&
                        <Form.Item
                            label={`Số phòng thi cần chọn là : ${Math.ceil((onFormCreate.countStudnetExam) / (onFormCreate.roomPeopleMax))} phòng`}
                            name="roomExam"
                        >
                            <Select
                                showSearch
                    ...
                    <div className='form_button_group'>
                        ...
                        <Form.Item wrapperCol={{ offset: 8, span: 16 }}>
                            <Button type="primary" htmlType="submit" onClick={handleOkCreate} disabled={countStudnetExam < 1 ? true : false}>
                                Submit
                            </Button>
                        </Form.Item>
                        <div />
