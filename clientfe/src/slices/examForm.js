...
const listExamForm = createSlice({
    name: 'getListExam',
    initialState,
    reducers: {
        loadding: state => {
...
export const { loadding, getListExamSuccess, deleteExamList, loaddingFailes, createExamReducer, searchData ,editData } = listExamForm.actions

export function apiGetListExam(alert) {
    return async dispatch => {
        dispatch(loadding())
        try {
            const response = await instance.get('/exam-form');
...
export default listExamForm.reducer