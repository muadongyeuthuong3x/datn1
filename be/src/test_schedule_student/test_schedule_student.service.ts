...
export class TestScheduleStudentService {
  ...
  async create(createTestScheduleStudentDto: any, res: any) {
    const {
      roomPeopleMax,
      mode,
      timeYearExamStart,
    ...
    let messageRes = ''
    if (id_edit) {
      messageRes = "Sửa lịch thi thành công"
    } else {
      messageRes = "Tạo thành công"
    }
    try {
  ...
  async update(updateTestScheduleStudentDto: any, res: any) {
    // const id = updateTestScheduleStudentDto.id_edit;
    // await this.testScheduleStudentRepository.delete(id);
    return await this.create(updateTestScheduleStudentDto, res);
