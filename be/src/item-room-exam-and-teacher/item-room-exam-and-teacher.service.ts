...
export class ItemRoomExamAndTeacherService {
  ...
  async create(
    ...
    const {
      id_query_test_schedule_student,
      roomExamAndTeacher,
      mode,
      id_exam_big_class,
      roomPeopleMax,
    ...
    try {
      for (let i = 0; i < roomExamAndTeacher.length; i++) {
        ...
        newDataItemRoomExamAndTeacher.id_testScheduleStudent =
          id_query_test_schedule_student;
        newDataItemRoomExamAndTeacher.time_start =
          roomExamAndTeacher[i].time_start_exam;
        newDataItemRoomExamAndTeacher.time_end =
          roomExamAndTeacher[i].time_end_exam;
        newDataItemRoomExamAndTeacher.id_class_query =
        ...
        let vtEnd =
          vtStart + Number(roomPeopleMax) < dataArrrayId.length
            ? vtStart + Number(roomPeopleMax)
            : dataArrrayId.length;
        for (let k = vtStart; k < vtEnd; k++) {
          arrayIdUpdate.push(dataArrrayId[k].id);
        }
        const dataCreate = await this.itemRoomExamAndTeacherRepository.save(
          newDataItemRoomExamAndTeacher,
        );
        ...
        await this.teacherTrackRepository.create({
          idDataCreate,
  ...
  update(
    id: number,
    updateItemRoomExamAndTeacherDto: UpdateItemRoomExamAndTeacherDto,
  ) {
  ...
  async findListTeachers(time_start: Date, time_end: Date, idUnLess: number[]) {
    ...
    const data: any = await this.itemRoomExamAndTeacherRepository
      .createQueryBuilder('item-room-exam-and-teacher')
      .select([
        'item-room-exam-and-teacher.id',
        'teacher-track.id_teacher_track_query',
      ])
      .where(
        'item-room-exam-and-teacher.time_start BETWEEN :startDate AND :endDate',
      )
      .orWhere(
        'item-room-exam-and-teacher.time_end BETWEEN :startDate AND :endDate',
      )
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i]?.id_teacherTrack.length; j++) {
        arrayIdTeacherUnLess.push(
          Number(data[i].id_teacherTrack[j].id_teacher_track_query),
        );
      }
    }
  ...
  async findListExitRoomOrTeacherByTime(dataProps: any, res: any) {
    const { time_start_exam, time_end_exam, room_exam, teacher_exam } =
      dataProps;
  ...
  async findCountRoom(time_start: Date, time_end: Date, id: number) {
    const countRoom: any = await this.itemRoomExamAndTeacherRepository
      .createQueryBuilder('item-room-exam-and-teacher')
      .where(
        'item-room-exam-and-teacher.time_start BETWEEN :startDate AND :endDate',
      )
      .andWhere('item-room-exam-and-teacher.id_Room = :id')
      .setParameters({
        startDate: new Date(time_start),
        endDate: new Date(time_end),
        id: id,
  ...
  async findCountTeacher(time_start: Date, time_end: Date, id: number) {
    const roomsList: any = await this.itemRoomExamAndTeacherRepository
      .createQueryBuilder('item-room-exam-and-teacher')
      .where(
        'item-room-exam-and-teacher.time_start BETWEEN :startDate AND :endDate',
      )
      ...
      .getMany();
    const countTeacher = await this.teacherTrackRepository.fintCountTeacher(
      roomsList,
      id,
    );
    return countTeacher;
  ...
  // async findListRoomsEdit(
  //   time_start: Date,
  //   time_end: Date,
  //   id: number,
  //   idUnLess: number[],
  // ) {
  //   const arrayIdRoomsUnless = [];
  //   // get ALL rooms and teachers time select
  //   const checkCount = await this.findCountRoom(time_start, time_end, id);
  //   const data: any = await this.itemRoomExamAndTeacherRepository
  //     .createQueryBuilder('item-room-exam-and-teacher')
  //     .select([
  //       'item-room-exam-and-teacher.id',
  ...
  //   for (let i = 0; i < data.length; i++) {
  //     if (checkCount > 2 && data.id == id) {
  //       continue;
  //     }
  //     arrayIdRoomsUnless.push(Number(data[i].id_class_query));
  //   }
  //   const dataUnLess = arrayIdRoomsUnless.concat(idUnLess);
  //   const dataSet = 
  //   const rooms = await this.rooms.findRoomExam(dataSet);
  //   return rooms;
  // }
}
